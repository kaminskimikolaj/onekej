...
function checkCredentials(username, password) {
	return new Promise(function(resolve, reject) {
			...
			request.get("https://ssl.dotpay.pl/s2/login/api/v1/accounts/", callback).auth(username, password)
			})
}
...
pathname: "/s2/login/api/v1/operations/",
...
protocol: 'https',
hostname: "ssl.dotpay.pl",
pathname: "/s2/login/api/v1/payments/" + paymentNumber + "/refund/",
})),
headers: {
"Accept": "application/json",
},
...
function checkRefundNumberForControlNumber(control) {
	return new Promise(function(resolve, reject) {
			...
			request.get(`https://ssl.dotpay.pl/s2/login/api/v1/operations/?type=refund&control=${control}`, callback).auth(apiCreds.username, apiCreds.password)
			})
}

function jsonstringifypretty(object) { return JSON.stringify(object).split('","').join('", "') }
			...
			function handleMultipleRecordsForControl() { 
				if (controlData.count == 0) {
					...
					eventsEmitter.emit('log', `${buffer.control} wont be refunded, found multiple records`)
						...
						let rejected = controlData.results.filter(record => record.status == "rejected")
						if(rejected) {
							rejected.forEach(rejectedRecord => {
								eventsEmitter.emit('log', `${buffer.control} with transaction number of ${rejectedRecord.number} is rejected operation`)
							})
						}
						...
						payments.forEach(paymentRecord => {
							...
							if (relatedOperations) {
								if (relatedOperations.length != 0) message += `refund/refunds: ${jsonstringifypretty(relatedOperations)}`
							}
